...
from safe_eth.eth.contracts import get_proxy_factory_V1_4_1_contract, get_safe_contract
...
class SafeData:
    ...
    guard: ChecksumAddress | None = None
    enabled_modules: list[ChecksumAddress] = None
    creation_block: int = 0
    creation_tx_hash: HexStr | None = None
    ...
    def __post_init__(self):
        ...
        if self.enabled_modules:
            self.enabled_modules = [
                fast_to_checksum_address(module) for module in self.enabled_modules
            ]
        ...
        if not self.creation_tx_hash:
            ...
            self.creation_tx_hash = HexStr(
                "0x" + hashlib.sha256(hash_input).hexdigest()
            )
        ...
        if self.threshold <= 0 or self.threshold > len(self.owners):
            raise ValueError(
                f"Invalid threshold {self.threshold} for {len(self.owners)} owners"
            )
    ...
    def from_onchain_state(
        ...
        try:
            ...
            raise ValueError(f"Cannot retrieve Safe info for {address}: {e}") from e
        except Exception as e:
            raise ValueError(f"Error reading Safe state for {address}: {e}") from e
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if from_onchain:
            self.stdout.write(
                self.style.SUCCESS(
                    f"Reading {len(from_onchain)} Safes from blockchain..."
                )
            )
            safes_to_process = []
            ...
            for safe_spec in from_onchain:
                ...
                try:
                    ...
                    safe_data = SafeData.from_onchain_state(
                        address=address, creation_block=creation_block
                    )
        ...
        for i, safe_data in enumerate(safes_to_process, 1):
            ...
            try:
                if dry_run:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"✓ Safe {safe_data.address} validation passed"
                        )
                    )
                else:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"✓ Safe {safe_data.address} inserted successfully"
                        )
                    )
            except Exception as e:
                self.stdout.write(
                    self.style.ERROR(
                        f"✗ Failed to process Safe {safe_data.address}: {e}"
                    )
                )
                continue
    ...
    def _insert_safe(self, safe_data: SafeData) -> None:
        ...
        ethereum_tx, created = EthereumTx.objects.get_or_create(
            ...
            defaults={
                ...
                "status": 1,  # Success
                "transaction_index": 0,
                "_from": safe_data.creator
                if safe_data.creator != "0x0000000000000000000000000000000000000000"
                else safe_data.owners[0],
                "gas": 0,
                "gas_price": 0,
                "nonce": 0,
                "to": safe_data.factory_address
                if safe_data.factory_address
        ...
        creation_internal_tx, created = InternalTx.objects.get_or_create(
            ...
            defaults={
                ...
                "_from": safe_data.factory_address
                if safe_data.factory_address
                != "0x0000000000000000000000000000000000000000"
                else safe_data.owners[0],
                "gas": 0,
        ...
        if setup_data and setup_data != HexBytes("0x"):
            self.stdout.write(
                "  Created setup InternalTx for SafeCreationView compatibility"
            )
